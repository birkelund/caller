...
// https://github.com/cockroachdb/cockroach/tree/master/pkg/util/caller) and
...
// from go1.8/src/go/build/build.go
...
func BuildPathStripper(depth int) *regexp.Regexp {
	...
	// Part of the regexp that matches `/depth0/depth1/depthN/.../(pkg/)`.
	pkgStrip := qSep + strings.Repeat(strings.Join([]string{"[^", "]+", ""}, qSep), depth) + "(?:pkg/)?(.*)"
...
var defaultRE = BuildPathStripper(3)

var DefaultCallResolver = NewCallResolver(defaultRE)

// Lookup returns the file, line and function of the caller at the requested
// depth, using a default call resolver which drops the path of the project
// repository.
